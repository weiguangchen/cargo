<template>
	<view class="page-bg" />
	<uv-navbar title="确认派单信息" bg-color="transparent" :title-style="{ color: '#FFFFFF' }" leftIconColor="#FFFFFF"
		placeholder @leftClick="leftClick" />
	...
	<view class="top-box">
		...
		<view class="select-time">
			<uv-image src="/static/images/dispatchTask/clock.png" width="36rpx" height="36rpx" :duration="0"
				:custom-style="{ marginRight: '24rpx' }" />
			<view class="time-wrapper">
				<view class="start-time" hover-class="time--active" hover-start-time="0" hover-stay-time="200" @click="selectTime('start')">
					<view class="time" v-if="startTime"  @click.stop>
						<text
							style="margin-right: 4rpx;font-weight: bold;">{{ dayjs(startTime).format('MM-DD HH:mm') }}</text>入场
						<uv-icon name="close" size="12" color="var(--intr-color)"
							:custom-style="{ marginLeft: '18rpx' }" @click="clearTime('start')" />
					</view>
					<view class="placeholder" v-else>
						车辆入场时间<uv-image src="/static/images/arrow.png" :duration="0" width="24rpx" height="24rpx"
							:custom-style="{ marginLeft: '6rpx' }" />
					</view>
				</view>
				<uv-line direction="col" length="24rpx" color="#B0BECC" margin="0 20rpx"></uv-line>
				<view class="end-time" hover-class="time--active" hover-start-time="0" hover-stay-time="200"  @click="selectTime('end')">
					<view class="time" v-if="endTime" @click.stop>
						<text
							style="margin-right: 4rpx;font-weight: bold;">{{ dayjs(endTime).format('MM-DD HH:mm') }}</text>结束
						<uv-icon name="close" size="12" color="var(--intr-color)"
							:custom-style="{ marginLeft: '18rpx' }" @click="clearTime('end')" />
					</view>
					<view class="placeholder" v-else>
						结束派单时间<uv-image src="/static/images/arrow.png" :duration="0" width="24rpx" height="24rpx"
							:custom-style="{ marginLeft: '6rpx' }" />
					</view>
				</view>
			</view>
	...
	<uv-form labelPosition="left" :model="model" :rules="rules" ref="form" label-width="200rpx">
		<view class="form-item-wrapper">
			<uv-form-item label="货主公司">
				<view style="display: flex;justify-content: flex-end;">
					<SelectCargo v-model="model.OwnerId" :options="cargoOptions" :disabled="cargoDisabled"
						@disabledClick="cargoDisabledClick" @change="cargoChange" />
				</view>
			</uv-form-item>
		</view>
		<view class="form-item-wrapper">
		...
		<view class="form-item-wrapper material">
			<view class="title-wrapper">
				...
				<view v-if="order" class="no" @click="openOrder">来自订单 {{ order.SSOrderNo }} <uv-image
						v-if="orderList.length > 1" src="/static/images/arrow.png" :duration="0" width="24rpx"
						height="24rpx" :custom-style="{ marginLeft: '6rpx' }" /></view>
			</view>
			<uv-form-item prop="material" labelPosition="top">
				<view class="select-material">
					<view v-if="!(supply && model.OwnerId)" class="tip">请先确认装货地和订单</view>
					<view v-else-if="orderList.length === 0" class="tip">未查询到生效中的订单</view>
					<view v-else-if="!order" class="tip" @click="openOrder">
						存在多笔订单，<text style="color: var(--main-color);">请选择</text><uv-image
							src="/static/images/dispatchTask/arrow2.png" :duration="0" width="16rpx" height="16rpx" />
					</view>
					<view v-else class="materials">
						<Material :borderBottom="index < order.MaterialsList.length - 1"
							v-for="(item,index) in order.MaterialsList" v-model="order.MaterialsList[index]" :order="order" :key="item.SSMaterialId" />
					</view>
				</view>
			</uv-form-item>
		</view>
	...
	<view class="page-footer">
		<view class="left">
			<view class="">
				预估<text class="price1">¥ {{ totalPrice }}</text>
			</view>
			<view class="">
				余额<text class="price2">¥ {{ order ? order.Balance ? order.Balance : 0.00 : '-'  }}</text>
			</view>
		</view>
		<view class="right">
			<uv-button :loading="submiting" text="确认派单" color="linear-gradient( 270deg, #31CE57 0%, #07B130 100%)"
				:customStyle="{ height: '96rpx' }" @click="submit" />
		</view>

		<view class="warning" v-if="showTip">预估价超出余额，存在车辆装货后不能出厂风险，请及时充值</view>
	</view>

	<!-- 时间 -->
	<uv-datetime-picker ref="datetimePicker" v-model="value" mode="datetime" confirmColor="var(--main-color)" :formatter="formatter" :maxDate="maxDate" :minDate="minDate" @confirm="confirmTime" />

	<!-- 订单 -->
	<my-drawer ref="orderDrawer" title="选择订单">
		<view class="my-dropdown-items">
			<view class="my-dropdown-item" :class="{'active': order && order.SSOrderId === item.SSOrderId}"
				v-for="item in orderList" :key="item.SSOrderId" @click="selectOrder(item)">
				<view style="flex:1;">订单 {{ item.SSOrderNo }}</view>
				<uv-image v-if="order && order.SSOrderId === item.SSOrderId" src="/static/images/check.png"
					:duration="0" width="32rpx" height="32rpx" />
			</view>
		</view>
	</my-drawer>
</template>
...
<script setup>
	...
	const showTip = computed(() => {
		...
		const total = Big(order.value.Balance || 0).plus(order.value.CreditBalance || 0).minus(totalPrice.value)
...
<style lang="scss">
	.bg {
		...
		z-index: -1;
		height: 476rpx;
		background: linear-gradient(270deg, #31CE57, #07B130);
	}
