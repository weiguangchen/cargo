	...
	<my-drawer ref="drawer">
		<template #title>
			<view style="display: flex;align-items: baseline;">
				选择车辆类型<text style="font-size: 24rpx;color: var(--intr-color);">（可多选）</text>
			</view>
		</template>
		<view class="my-dropdown-items">
			<view class="my-dropdown-item" :class="{'active': innerValue.includes(item.value) }" v-for="item in options"
				@click="select(item)">
				<view class="car-type">
					<uv-image v-if="item.value === '自卸车'" src="/static/images/carType/car1.png" width="100%" height="100%" :duration="0"/>
					<uv-image v-if="item.value === '半挂车'" src="/static/images/carType/car2.png" width="100%" height="100%" :duration="0"/>
					<uv-image v-if="item.value === '罐车'" src="/static/images/carType/car3.png" width="100%" height="100%" :duration="0"/>
				</view>
				<view class="name">{{ item.label }}</view>
				<uv-image v-if="innerValue.includes(item.value)" src="/static/images/check.png" :duration="0"
		...
		<template #footer>
			<view class="btns">
				...
				<view class="right" @click="confirm">
					<uv-button text="确认所选" color="linear-gradient( 270deg, #31CE57 0%, #07B130 100%);"
