<template>
	<view @click="showDrawer" class="input-wrapper">
		<view :class="{ 'placeholder': !text, 'text': text }">{{ text || '请选择' }}</view>
	...
	<my-drawer ref="drawer">
		...
		<view class="my-dropdown-items">
			<view class="my-dropdown-item" :class="{'active': innerValue === item.value }" v-for="item in options"
				@click="select(item)">
				<view class="name">{{ item.label }}</view>
				<uv-image v-if="innerValue === item.value" src="/static/images/check.png" :duration="0"
