<template>
	<view class="bill" @click="toDetail">
		<view class="tag" :class="{ 'process': ['0','1','2','3','4','5','6','7'].includes(props.record.Weightedstatus), 'finish': ['8','9'].includes(props.record.Weightedstatus) }">{{ billStatusName }}</view>
		<view class="material-name">{{ record.Materialname }}</view>
		<view class="weight">
			<template v-if="['0','1','2','3','4','5','9'].includes(record.Weightedstatus)">
		...
		<view class="location-wrapper">
			...
			<view class="to">
				...
				<view class="name" v-if="record.UnloadEnt && record.UnloadEnt.PlaceName">
					{{ record.UnloadEnt.PlaceName }}
				</view>
			</view>
		</view>
		<view class="price-wrapper" v-if="record.TaxPrice">
			<view class="price" v-if="['0','1','2','3','4','5','9'].includes(props.record.Weightedstatus)">
				预估总价 <text style="font-weight: bold;">¥ {{ record.EstimateAmount }}</text>
			</view>
			<view class="price" v-if="['6'].includes(props.record.Weightedstatus)">
				待结算 <text style="font-weight: bold;">¥ {{ record.RealAmount }}</text>
			</view>
			<view class="price" v-if="['7','8'].includes(props.record.Weightedstatus)">
				已支付 <my-price color="var(--red-color)" :price="record.RealAmount"/>
			</view>
		</view>
		<template v-if="['0','1','2','3','4','5','6','7'].includes(props.record.Weightedstatus)">
			<uv-line color="var(--divider)" margin="24rpx 0" />
			<view class="footer" >
				<view class="btn" @click.stop v-if="['0','1'].includes(props.record.Weightedstatus)">
					<uv-button shape="circle" text="取消运单" color="var(--page-bg)"
						:customTextStyle="{ color: 'var(--content-color)', fontSize: '26rpx' }"
						:customStyle="{ height: '32px' }" @click="cancel"/>
				</view>
				<view class="btn" @click.stop v-if="['0','1','2','3','4','5','6','7'].includes(props.record.Weightedstatus)">
					<uv-button shape="circle" text="呼叫司机" color="var(--page-bg)"
						:customTextStyle="{ color: 'var(--content-color)', fontSize: '26rpx' }"
						:customStyle="{ height: '32px' }" @click="takePhone" />
				</view>
				<view class="btn" @click.stop v-if="['7'].includes(props.record.Weightedstatus)">
