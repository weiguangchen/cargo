<template>
  ...
  >
    <view class="form-title"
      >卸货地信息 <text v-if="model.Id">（不可修改）</text></view
    >
    <view class="form-wrapper">
      ...
      <uv-form-item
        ...
        label="地区"
        borderBottom
        v-if="model.Area.province"
        :custom-style="{ alignItems: 'center' }"
      >
        <text style="padding-left: 4px"
          >{{ model.Area.province }} {{ model.Area.city }}
          {{ model.Area.district }}</text
        >
      </uv-form-item>
      <uv-form-item
        v-if="model.Id"
      ...
      >
        <uv-input
          v-model="model.Area.address"
          placeholder="请选择，提交后不可修改"
          placeholder-style="color:var(--intr-color)"
          border="none"
          readonly
        />
        <template v-slot:right>
          <uv-icon
            name="arrow-right"
            :custom-style="{ color: 'var(--intr-color)' }"
            size="12"
          />
        </template>
      </uv-form-item>
  ...
  <view class="page-footer">
    <view style="width: 262rpx" v-if="model.Id">
      <uv-button
        ...
        :custom-text-style="{ color: 'var(--red-color)' }"
        color="var(--red-color)"
        @click="remove"
      />
    </view>
