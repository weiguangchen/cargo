  ...
  <view class="bill" @click="toDetail" v-if="record">
    ...
    <view class="weight">
      <template v-if="!!record.EstimateCarWeight && !!record.EstimateCarTimes">
        已装运 {{ record.RealCarWeight || 0.0 }}/{{
          record.EstimateCarWeight || 0.0
        }}
        吨 + {{ record.RealCarTimes || 0.0 }}/{{
          record.EstimateCarTimes || 0.0
        }}
        车次
      </template>
      <template v-else-if="!!record.EstimateCarTimes">
        已装运 {{ record.RealCarTimes || 0.0 }}/{{
          record.EstimateCarTimes || 0.0
        }}
        车次
      </template>
    ...
    <view class="price-wrapper">
      ...
      <view class="price">
        ...
        <text style="font-weight: bold"
          >¥ {{ record.EstimateCarAmount || 0.0 }}</text
        >
      </view>
    </view>
      ...
      try {
        ...
        uni.showToast({
          ...
          complete() {
            ...
            uni.$emit(`manifest:finish`, props.record);
          },
        });
        confirm.value.close();
      } catch (err) {
      ...
      try {
        ...
        uni.showToast({
          ...
          complete() {
            ...
            uni.$emit(`manifest:pause`, props.record);
          },
        });
        confirm.value.close();
      } catch (err) {
    ...
    async confirm() {
      try {
        ...
        await uni.showToast({
          ...
          icon: "none",
          complete() {
            uni.$emit(`taskManifest:goOn`, props.record);
            uni.$emit(`manifest:goOn`, props.record);
          },
        });
        confirm.value.close();
      } catch (err) {
