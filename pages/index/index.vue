<template>
	...
	<view class="dispatch-wrapper">
		<uv-image src="/static/images/home/logo.png" width="488rpx" height="144rpx" :duration="0" />
		<view class="task-wrapper">
			<view class="content green" v-if="!getToken()" @click="openLoginDrawer">
				派单前需要先完成登录
				<uv-image src="/static/images/dispatchTask/arrow2.png" width="24rpx" height="24rpx" :duration="0"/>
			</view>
			<view class="content green" v-else-if="count === 0">暂无进行中任务，快派单吧！</view>
			<view class="content orange" v-else>
				有 {{ count }} 项进行中的运输任务
				<uv-image src="/static/images/orange-arrow.png" width="24rpx" height="24rpx" :duration="0"/>
			</view>
		</view>
		<view class="form-wrapper">
			<view class="select-location">
				<view class="from-wrapper" @click="selectAddress(1)">
					...
					<view class="info">
						<view class="name uv-line-2" v-if="supply">{{ supply.Name }}</view>
				...
				<view class="to-wrapper" @click="selectAddress(2)">
					...
					<view class="info">
						<view class="name uv-line-2" v-if="unload">{{ unload.Placename }}</view>
			...
			<uv-button text="立即派单" color="linear-gradient( 270deg, #31CE57 0%, #07B130 100%)"
				:custom-style="{ height: '104rpx', fontWeight: 'bold', borderRadius: '16rpx', marginBottom: '24rpx' }" :customTextStyle="{ fontSize: '32rpx' }"
				@click="dispatch" />
	...
	<view class="banner" @click="follow">
		<uv-image width="100%" height="100%" :duration="0" src="/static/images/home/banner.png"/>
	</view>
	<!-- end -->
	<!-- 登录弹窗 -->
...
<style lang="scss">
	...
	.dispatch-wrapper {
		...
		.task-wrapper {
			...
			position: relative;
			height: 160rpx;
			background: url(/static/images/home/task-bg.png) left bottom/auto 108rpx no-repeat,
			...
			.content {
				...
				&.orange {
					color: #FC7E2C;
					background: linear-gradient( 90deg, #FFF1E1 65%, rgba(255,242,233,0) 100%);
				}
			}
		}
