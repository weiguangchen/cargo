<template>
  ...
  <view class="status-wrapper" @click="openStepModal">
    ...
    <view class="status-tip">
      <template v-if="detail.Weightedstatus === '0'">
        ...
        <template
          v-else-if="
            ...
            (detail.StartTime &&
              dayjs().isAfter(detail.StartTime) &&
              !detail.EndTime)
          "
          >等待司机入厂装货</template
        >
        <template
          v-else-if="
            !detail.EndTime &&
            detail.StartTime &&
            dayjs().isBefore(detail.StartTime)
          "
        >
        ...
        <template
          ...
          "
        >
          {{ dayjs(detail.StartTime).format("MM-DD HH:mm") }} 至
          {{ dayjs(detail.EndTime).format("MM-DD HH:mm") }} 司机可入厂装货
        </template>
        <template
          v-else-if="
            (detail.StartTime &&
      ...
      <template v-if="detail.Weightedstatus === '1'">{{
        detail.StatusCount && detail.StatusCount > 1
          ? `当前共 ${detail.StatusCount} 辆车等待入厂`
          : "即将入厂"
      }}</template>
      <template v-if="detail.Weightedstatus === '2'">请遵守厂内规范</template>
      <template v-if="detail.Weightedstatus === '3'">{{
        detail.StatusCount && detail.StatusCount > 1
          ? `当前共 ${detail.StatusCount} 辆车等待装车`
          : "即将装车"
      }}</template>
      <template v-if="detail.Weightedstatus === '4'">请进行物料装车</template>
      <template v-if="detail.Weightedstatus === '5'"
        >请根据提示信息进行装车</template
      >
      <template v-if="detail.Weightedstatus === '6'"
        >请及时出厂并前往卸货地完成卸货</template
      >
      <template v-if="detail.Weightedstatus === '7'"
        >请前往卸货地完成卸货</template
      >
      <template v-if="detail.Weightedstatus === '8'"
        >{{
          detail.UserType
      ...
      >
      <template v-if="detail.Weightedstatus === '9'"
        >{{
          detail.UserType
            ? detail.UserType === "driver"
              ? "司机"
  ...
  <view class="location-wrapper">
    <view class="from-wrapper" @click="openMapModal(1)" v-if="detail.SupEnt">
      ...
      <view
        class="content-box"
        :class="{ 'my-border-bottom': detail.UnloadPlace }"
      >
        <view class="info" @click="selectAddress">
          <view class="name uv-line-1">{{ detail.SupEnt.Name || "" }}</view>
          <view class="address uv-line-1">
            <template
              v-if="!detail.SupEnt.Linker & !detail.Supplier.LinkerMobile"
            >
              暂无联系人信息
            </template>
            <template v-else>
              <text style="margin-right: 15px" v-if="detail.Supplier.Linker">{{
                detail.Supplier.Linker || ""
              }}</text>
              <text v-if="detail.Supplier.LinkerMobile">{{
                detail.Supplier.LinkerMobile || ""
              }}</text>
            </template>
          </view>
        </view>
    ...
    <view class="to-wrapper" @click="openMapModal(2)" v-if="detail.UnloadEnt">
      ...
      <view class="content-box">
        <view class="info" @click="selectAddress">
          <view
            class="name uv-line-1"
            v-if="detail.UnloadEnt && detail.UnloadEnt.PlaceName"
            >{{ detail.UnloadEnt.PlaceName }}</view
          >
          <view class="address uv-line-1">
            <template
              v-if="!detail.UnloadEnt.NickName & !detail.UnloadEnt.Mobile"
            >
              暂无联系人信息
            </template>
            <template v-else>
              <text
                ...
                v-if="detail.UnloadEnt.NickName"
                >{{ detail.UnloadEnt.NickName || "" }}</text
              >
              <text v-if="detail.UnloadEnt.Mobile">{{
                detail.UnloadEnt.Mobile || ""
              }}</text>
            </template>
          </view>
        </view>
  ...
  <view class="bangdan" v-if="detail.WeightEnt && detail.WeightEnt.BillNo">
    <view class="left">
      ...
      <view class="no"
        >磅单号：{{ detail.WeightEnt ? detail.WeightEnt.BillNo : "" }}</view
      >
    </view>
    <!-- <view class="right">
  ...
  <view class="bill-car">
    <view
      class="car-info-wrapper"
      :class="{ 'my-border-bottom': detail.FirstWeight }"
      v-if="detail.Carno"
    >
      <view class="info">
      ...
      <view class="person">
        <text
          class="user"
          v-if="detail.DriverEnt && detail.DriverEnt.NickName"
          >{{ detail.DriverEnt.NickName }}</text
        >
        <text
          class="phone"
          v-if="detail.DriverEnt && detail.DriverEnt.Mobile"
          >{{ detail.DriverEnt.Mobile }}</text
        >
      </view>
    </view>
    <view
      v-if="['4', '5', '6', '7', '8'].includes(detail.Weightedstatus)"
      class="weight"
      :class="{ 'my-border-bottom': detail.SecondWeight }"
    >
      <view class="label" v-if="detail.WeightEnt"
        >皮重 {{ detail.WeightEnt.FirstWeight || "" }} 吨</view
      >
      <view class="value" v-if="detail.WeightEnt"
        >测量于
        {{
          detail.WeightEnt.FirstTime
            ? dayjs(detail.WeightEnt.FirstTime).format("MM/DD HH:mm")
            : ""
        }}</view
      >
    </view>
    <view v-if="['6', '7', '8'].includes(detail.Weightedstatus)" class="weight">
      <view class="label" v-if="detail.WeightEnt"
        >毛重 {{ detail.WeightEnt.SecondWeight || "" }} 吨</view
      >
      <view class="value" v-if="detail.WeightEnt"
        >测量于
        {{
          detail.WeightEnt.SecondTime
            ? dayjs(detail.WeightEnt.SecondTime).format("MM/DD HH:mm")
            : ""
        }}</view
      >
    </view>
  ...
  <view class="info-wrapper">
    ...
    <view
      class="item my-border-top"
      v-if="['8', '9'].includes(detail.Weightedstatus)"
    >
      <view class="label">{{
        detail.Weightedstatus == "8" ? "完成时间" : "取消时间"
      }}</view>
    ...
    <view
      class="item my-border-top"
      v-if="detail.UnloadMemo && detail.Weightedstatus == '8'"
    >
  ...
  <view
    class="page-footer"
    v-if="
      ['0', '1', '2', '3', '4', '5', '6', '7'].includes(detail.Weightedstatus)
    "
  >
    ...
    <view
      class="btns"
      v-if="['2', '3', '4', '5', '6'].includes(detail.Weightedstatus)"
    >
      <uv-button
        text="呼叫司机"
        color=" linear-gradient( 270deg, #31CE57 0%, #07B130 100%)"
...
function openMapModal(type) {
  const data = {
    ...
    name:
      type === 1
        ? detail.value?.SupEnt?.Name ?? ""
        : detail.value?.UnloadEnt?.PlaceName ?? "",
    address:
      type === 1
        ? `${detail.value?.SupEnt?.Province ?? ""}${
            detail.value?.SupEnt?.City ?? ""
          }${detail.value?.SupEnt?.District ?? ""}${
            detail.value?.SupEnt?.Address ?? ""
          }`
        : `${detail.value?.UnloadEnt?.Province ?? ""}${
            detail.value?.UnloadEnt?.City ?? ""
          }${detail.value?.UnloadEnt?.District ?? ""}${
            detail.value?.UnloadEnt?.Address ?? ""
          }`,
    user:
      type === 1
        ? detail.value?.SupEnt?.Linker ?? ""
        : detail.value?.UnloadEnt?.NickName ?? "",
    phone:
      type === 1
        ? detail.value?.SupEnt?.LinkerMobile ?? ""
        : detail.value?.UnloadEnt?.Mobile ?? "",
    longitude:
      type === 1
        ? detail.value?.Supplier?.Logitude
        : detail.value?.UnloadEnt?.Logitude,
    latitude:
      type === 1
        ? detail.value?.Supplier?.Latitude
        : detail.value?.UnloadEnt?.Latitude,
  };
  ...
  confirm.value.confirm({
    ...
    async confirm() {
      try {
        ...
        uni.$emit(`taskWaybill:cancel`, detail.value);
        uni.$emit(`waybill:cancel`, detail.value);
        getDetail();
        getTimeSteps();
      } catch (err) {
  ...
  confirm.value.confirm({
    ...
    async confirm() {
      try {
        ...
        uni.$emit(`taskWaybill:confirmUnload`, detail.value);
        uni.$emit(`waybill:confirmUnload`, detail.value);
        getDetail();
        getTimeSteps();
      } catch (err) {
        console.log("err", err);
